...
<body>
  ...
  <form action="https://formsubmit.co/trashcicle@gmail.com" method="POST" class="flex flex-col gap-4 max-w-md w-full">
 
  <input type="hidden" name="_captcha" value="false">

  
  <input type="hidden" name="_next" value="https://formulario-contacto.php/formulario-contacto.php">
  ...
  <input name="nombre" class="bg-[--oscuro] border border-[var(--primario)] rounded-md px-3 py-2 text-xs text-[var(--claro)] placeholder-[var(--claro)]/60 focus:outline-none" id="campo-nombre" placeholder="Nombre..." type="text" required />

  <label class="text-[var(--primario)] text-xs font-semibold" for="campo-correo">Correo electrónico</label>
  <input name="email" class="bg-[--oscuro] border border-[var(--primario)] rounded-md px-3 py-2 text-xs text-[var(--claro)] placeholder-[var(--claro)]/60 focus:outline-none" id="campo-correo" placeholder="Correo..." type="email" required />

  <label class="text-[var(--primario)] text-xs font-semibold" for="campo-mensaje">Mensaje</label>
  <textarea name="mensaje" class="bg-[--oscuro] border border-[var(--primario)] rounded-md px-3 py-2 text-xs text-[var(--claro)] placeholder-[var(--claro)]/60 resize-none focus:outline-none" id="campo-mensaje" placeholder="Mensaje..." rows="4" required></textarea>
  ...
  <a href="index.php" class="group fixed bottom-4 left-4 bg-[var(--primario)] text-[--oscuro] pl-4 pr-4 py-3 rounded-full shadow-lg hover:bg-[var(--primario-oscuro)] transition-all duration-300 flex items-center overflow-hidden max-w-[48px] hover:max-w-[200px]">
      ...
      if (errores.length > 0) {
        ...
        .then(resultado => {
          if (resultado.trim() === 'success') {
            ...
            mostrarAlerta('Hubo un error al enviar el mensaje. Intenta nuevamente.');
          }
        })
        .catch(() => {
          mostrarAlerta('Error en la conexión. Intenta más tarde.');
        });
      }
    });
