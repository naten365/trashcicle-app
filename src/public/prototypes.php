...
$sql = "SELECT id_zafacon, nombre_zafacon, ubicacion, capacidad, residuos_actuales, estado, fecha_instalacion FROM zafacones_inteligentes";
$stmt = $pdo->query($sql);
...
<body>
  ...
  <div id="device-list">
    ...
    <button class="btn-view-more">VER UBICACIÓN</button>
        </div>
      </div>
    <?php endforeach; ?>
  </div>
  ...
  button.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const response = await fetch('toggle-status.php', {
        ...
        body: JSON.stringify({ id: deviceId, status: newStatus === 'enabled' ? 1 : 0 }),
      });
